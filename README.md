...
| :--- | :--- | :--- |
| **[patent-low-bandwidth-ai](https://github.com/eauchs/patent-low-bandwidth-ai)** | Patent / Backend | **(Patent Pending)** A hybrid RAG backend that enables stateful AI conversations over low-bandwidth networks like SMS. |
| **[gui-agent](https://github.com/eauchs/gui-agent)** | AI Agents | A two-layer autonomous agent for macOS, separating visual perception (VLM) from strategic decision-making (LLM). |
| **[speech-to-speech-pipeline](https://github.com/eauchs/speech-to-speech-pipeline)** | MLOps / Real-Time | A low-latency (STT-LLM-TTS) conversational pipeline with barge-in, optimized for Apple Silicon (`MLX`). |
| **[enigma-shell](https://github.com/eauchs/enigma-shell)** | Full-Stack AI | An experimental web shell to control a full Linux VM (`v86`) using natural language via local LLMs (`Ollama`). |
